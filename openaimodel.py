    ...
    def forward(
        ...
        **kwargs,
        ...
        for module in self.output_blocks:
            ...
            if use_freeu:
                # breakpoint() # dimensions may have changed, so just in case.
                if h.shape[1] == 1280:
                    h[:, :640] = h[:, :640] * self.b1
                    hs_ = self.Fourier_filter(hs_, threshold=1, scale=self.s1)
                if h.shape[1] == 640:
                    h[:, :320] = h[:, :320] * self.b2
                    hs_ = self.Fourier_filter(hs_, threshold=1, scale=self.s2)
                # FreeU code end
